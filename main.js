...
// Helper: set flag image by element id and country code (flagcdn code)
// Prefer SVG, fall back to PNG if SVG fails to load; handle <img> or inline <svg> targets.
function setFlagImage(elementId, countryCode, altText = '') {
  const el = document.getElementById(elementId);
  if (!el) return;
  const svgUrl = `https://flagcdn.com/${countryCode}.svg`;
  const pngUrl = `https://flagcdn.com/w40/${countryCode}.png`;
  const tag = el.tagName && el.tagName.toLowerCase();
  if (tag === 'img') {
    el.src = svgUrl;
    el.onerror = () => { el.onerror = null; el.src = pngUrl; };
  ...
  if (tag === 'svg') {
    ...
    img.id = el.id;
    img.src = svgUrl;
    img.onerror = () => { img.onerror = null; img.src = pngUrl; };
    img.alt = altText || elementId;
    el.parentNode.replaceChild(img, el);
    return;
  }

  // Fallback for other elements: set background image
...
function saveSettings() {
  try {
    const settings = {
      fromCurrency,
      toCurrency,
      amountFrom: amountFromInput.value
    };
...
function loadSettings() {
  try {
    ...
    if (settings.fromCurrency && currencyFlags[settings.fromCurrency]) {
      ...
      setFlagImage('fromFlag', currencyFlags[fromCurrency], fromCurrency);
    }
    if (settings.toCurrency && currencyFlags[settings.toCurrency]) {
      toCurrency = settings.toCurrency;
      const toCodeEl = document.getElementById('toCode');
      if (toCodeEl) toCodeEl.textContent = toCurrency;
      setFlagImage('toFlag', currencyFlags[toCurrency], toCurrency);
    }
    if (settings.amountFrom !== undefined && !isNaN(parseFloat(settings.amountFrom))) {
      amountFromInput.value = parseFloat(settings.amountFrom).toFixed(2);
    }
  } catch (e) {
    // ignore parse errors
  }
...
function showCurrencyPicker(type) {
  ...
  currencies.forEach(currency => {
    ...
    item.innerHTML = `
      <img class="flag" src="https://flagcdn.com/${currencyFlags[currency]}.svg" alt="${currency}">
...
function selectCurrency(currency) {
  if (currentPickerType === 'from') {
    ...
    setFlagImage('fromFlag', currencyFlags[currency], currency);
    document.getElementById('fromCode').textContent = currency;
  } else {
    toCurrency = currency;
    setFlagImage('toFlag', currencyFlags[currency], currency);
...
amountFromInput.addEventListener('input', () => { convertCurrency(); saveSettings(); });
...
window.addEventListener('load', () => {
  loadSettings();
  // Ensure flags show current selection after loading settings
  setFlagImage('fromFlag', currencyFlags[fromCurrency], fromCurrency);
  setFlagImage('toFlag', currencyFlags[toCurrency], toCurrency);
  fetchRates();
  convertCurrency();
